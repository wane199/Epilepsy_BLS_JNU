...
# 1. 创建任务
task <- as_task_classif(dat, target = "oneyr")
task
...
predictions$score(msr("classif.auc")) # 预测准确率
...
tasks <- as_task_classif(dat, target = "oneyr")
...
rr$score(msr("classif.auc")) # 各个重抽样的AUC
...
# 将特征工程用于新数据
graph$predict(tasks$filter(1:234))[[1]]$data()
...
mod <- Predictor$new(learner, data = dat, y = "oneyr")
...
mod <- Predictor$new(learner, data = dat, y = "oneyr")
shapley <- Shapley$new(mod, x.interest = dat[10, ])
plot(shapley)
# 7.1.3特征重要度
...
ranger_exp <- explain_mlr3(ranger,
  data = dat, y = as.numeric(as.character(dat$oneyr)),
...
dat_pd
plot(dat_pd) +
  scale_y_continuous("TLE Relapse Prediction Probability(%)", # TLE Prediction Probability(%)
...
sub1_shap <- predict_parts(ranger_exp,
  new_observation = sub1,
  type = "shap"
)
plot(sub1_shap) +
