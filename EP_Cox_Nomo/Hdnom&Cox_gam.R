# R语言hdnom包进行高维惩罚 Cox 回归模型绘制列线图-校准曲线-时间依赖ROC-外部验证(https://mp.weixin.qq.com/s?__biz=MzI1NjM3NTE1NQ==&mid=2247487243&idx=1&sn=ac3feba208a3fae29faff2dd4d7436b5&chksm=ea26ef17dd5166014a009711662c91580ea54a0dad6e82042acdf401dfbeb45ec961a705b047&mpshare=1&scene=1&srcid=1227Exc3fYEWFp2ktsXmNDwp&sharer_sharetime=1672111595519&sharer_shareid=13c9050caaa8b93ff320bbf2c743f00b#rd)
rm(list = ls())
options(digits = 3) # 限定输出小数点后数字的位数为3位
library("hdnom")
dt <- read.csv("C:\\Users\\wane199\\Desktop\\sci\\cph\\TLE234group.csv", sep = ",", header = TRUE)
train <- subset(dt, dt$Group == "Training")
test <- subset(dt, dt$Group == "Test")

x <- as.matrix(train[, -c(1:5)])
time <- train$Follow_up_timemon
event <- train$Rel._in_5yrs
y <- survival::Surv(time, event)

# 使用弹性网络需要加载doParallel包
suppressMessages(library("doParallel"))
registerDoParallel(detectCores())


fit <- fit_aenet(x, y, nfolds = 5, rule = "lambda.min", seed = c(123, 123), parallel = TRUE)
...
nom <- as_nomogram(
  fit, x, time, event,
  pred.at = 12 * 1,
  funlabel = "2-Year Overall Survival
Probability"
)
# 绘图
print(nom)
...
# 进行模型验证，先进行内部验证，使用validate函数进行
val_int <- validate(
  x, time, event,
  model.type = "aenet",
  alpha = alpha, lambda = lambda, pen.factor =
    adapen,
  method = "bootstrap", boot.times =
    10,
  tauc.type = "UNO", tauc.time =
    seq(1, 5, 0.5) * 12,
...
plot(b$family$data$tr, exp(-b$family$data$h),
  type = "l", ylim = c(0, 1),
...
ph.resid <- function(b, stratum = 1) {
  ...
  for (i in 1:length(term)) {
    ii <- term[[i]]
    m <- length(ii)
    plot(b$y[oy], rs[oy, ii[1]],
      ylim = c(-3, 3), type = "l", ylab = "score residuals",
      xlab = "time", main = names(term)[i]
    )
    if (m > 1) for (k in 2:m) lines(b$y[oy], rs[oy, ii[k]], col = k)
    abline(-1.3581, 0, lty = 2)
    abline(1.3581, 0, lty = 2)
  }
}
par(mfrow = c(2, 2))
...
for (i in 1:2) { ## loop over strata
  ...
  if (i == 1) {
    plot(bs$family$data$tr[ind], exp(-bs$family$data$h[ind]),
      type = "l",
      ylim = c(0, 1), xlab = "time", ylab = "survival", lwd = 2, col = i
    )
  } else {
...
mg <- gam(cbind(dumt, stratum) ~ spontaneous + induced,
  data = infert,
  family = cox.ph, weights = case
)
ms <- clogit(case ~ spontaneous + induced + strata(stratum),
  data = infert,
  method = "approximate"
)
summary(mg)$p.table[1:2, ]
ms
