...
library(modEvA)
aupr <- AUC(
  obs = train$Y, pred = pre, interval = 0.001,
...
library(rmda)
set.seed(123)
baseline.model <- decision_curve(Y ~ .,
  data = train,
...
plotPredictivenessCurve(
  predrisk = cbind(predRisk1, predRisk2),
