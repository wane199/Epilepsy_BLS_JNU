...
bool <- stringr::str_detect(
  string = res_ALL3$p_no,
...
writeData(wb,
  x = res_ALL4, sheet = "sheet1",
  rowNames = FALSE
)
writeData(wb,
  x = res_ALL4_2, sheet = "sheet2",
  rowNames = FALSE
)
## style for body
...
# 3. 数据预处理
# 合并暴露和结局的数据:对数据进行预处理，使其效应等位与效应量保持统一，就是调整exp和out上位点的方向和beta，使其统一。
